...
def marker_detection(raw_image_blur):
    ...
    mask = (diff[:, :, 0] > 5) & (diff[:, :, 2] > 30) & (diff[:, :, 1] > 50) #超參,輸出爲1/0陣  #超參
...
#    ref_blur_small = cv2.pyrDown(ref_blur).astype(np.float32)
    ...
    # depth1[depth1<0] = 0
        depth2[depth2<0] = 0
        depth_list.append(depth2)
        # cloud_img = show_depth3d(depth2)
        ...
        #     break
    cam.release()
    np.save(path + 'video/raw_img.npy', raw_img_list)
    np.save(path + 'video/depth.npy', depth_list)
